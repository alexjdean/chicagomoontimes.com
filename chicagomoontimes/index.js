...
exports.generateArticle = functions.pubsub.schedule('every 1 minutes').onRun(async (context) => {
  const database = admin.database();
  const articlesRef = database.ref('articles');
  
  const hasGenerated = await database.ref('hasGenerated').once('value').then(snapshot => snapshot.val());
  if (hasGenerated) {
  ...
  axios.get('http://localhost:3001/article')
    .then(async (response) => {
      ...
      const nationalImage = await getUnsplashImage(nationalObj.keyword, config.apiKey);
      const chicagoImage = await getUnsplashImage(chicagoObj.keyword, config.apiKey);
